...
import getTrendContext, { MonthlyPoint } from './trendContext';

const MARGIN = { top: 12, right: 16, bottom: 28, left: 52 };
const CLIP_ID = 'trend-clip';
const X_MAX = 2025.75; // always show through end of 2025
...
function allQuarterlyTicks(xMin: number): number[] {
  const ticks: number[] = [];
  for (let yr = Math.ceil(xMin); yr <= 2025; yr++) {
...
function TrendComponent() {
  ...
  useEffect(() => {
    ...
    const maxRate = d3.max(points, d => d.accident_rate) ?? 0.01;
    ...
    const years = d3.range(Math.ceil(xMin), 2026);
    ...
    const dots = chartG.selectAll<SVGCircleElement, QuarterlyPoint>('circle')
